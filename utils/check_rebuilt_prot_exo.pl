...
if (-t STDIN) {
	print <<HELP;
sed 1d exo.best.tsv | awk -v cov=90 -v id=90 '\$7>=cov&&\$8>=id' | check_rebuilt_prot_exo.pl <-nostat> <-chim> <-fusion> <-prot> <-hit> <-gene> <-f ensembl.pep.all.fa>
 -nostat: don't print statistics line
 -chim: print putative chimeric contigs
 -fusion: print putative fusion contigs
 -prot: print matching proteins (could be used with -chim or -fusion or both)
 -hit: print contigs hits (could be used with -chim or -fusion or both)
 -gene: print genes matching contigs (need -f option)
HELP
	exit;
}
...
while (<STDIN>) {
	...
	my ($start, $stop) = $tmp[$pos{strand}] eq '-' ? ($tmp[$pos{stop}], $tmp[$pos{start}]) : ($tmp[$pos{start}], $tmp[$pos{stop}]);
	my $hitLength = $stop - $start + 1;
	$tlength{$tmp[$pos{target}]} = $tmp[$pos{tlength}];
