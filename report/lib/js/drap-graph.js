 ...
 * @param pXCategories {Array} x scale labels.
 * @param pData {Array} The HightChart series.
 * @param pLabelSize {Int} The minimum bar width to display the dataLabels.
 * @return {Hash} Parameters to use in Highchart's constructor.
 */
var barplot = function( pTitle, pXTitle, pYTitle, pXCategories, pData, pLabelSize ) {
	...
	// Hide dataLabels if bar width is too small
	if( pLabelSize !== undefined && pLabelSize != null ){
		chart['events'] = {
			load : function () {
				chart = this ;
				var toEnabled = false ;
				for( var idx in chart.series ){
					if( chart.series[idx].visible ){
						toEnabled = chart.series[idx].barW >= pLabelSize ;
					}
				}
				if( toEnabled ){
					$(chart.container).find('.highcharts-data-labels').show();
				} else {
					$(chart.container).find('.highcharts-data-labels').hide();
				}
			},
			redraw : function () {
				chart = this ;
